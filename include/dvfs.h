...
/**
 * dvfs_status() - print current OPP status
 *
 * @dev: dvfs device
 * @return 0 if OK, otherwise on error
 */
int dvfs_status(struct udevice *dev);

/**
 * dvfs_set_max() - set to maximum OPP
 *
 * @dev: dvfs device
 * @return 0 if OK, otherwise on error
 */
int dvfs_set_max(struct udevice *dev);
...
struct dm_dvfs_ops {
	...
	int (*status)(struct udevice *dev);
	int (*set_max)(struct udevice *dev);
};
