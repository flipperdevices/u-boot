...
spl_start_uboot()
    ...
    implementation always start the kernel. It may be called from more than one
    place in a single SPL run, so prefer spl_falcon_boot() when querying the
    result - that caches the answer, which matters for implementations that are
    not idempotent, such as those sampling a button.
...
and, outside secure Falcon mode, falls back to ``os = "u-boot"``. The kernel
is entered at EL2 with the device tree address in x0, as the arm64 boot
protocol requires.
...
CONFIG_SPL_UFS_RAW_OS_DEVNUM and CONFIG_SPL_UFS_RAW_OS_SECTOR versus
...
Configure the board with the rockchip-falcon.config fragment, which enables
Falcon mode, makes it the default choice and turns on image generation::

        make <board>_defconfig rockchip-falcon.config

That is the equivalent of setting the following in the board defconfig::

        CONFIG_SPL_OS_BOOT=y
        CONFIG_SPL_OS_BOOT_DEFAULT=y
        CONFIG_ROCKCHIP_FALCON_IMAGE=y

The load addresses of the kernel, device tree and initramfs default to
offsets from the start of DRAM which clear BL31, OP-TEE and the staged FIT,
and can be adjusted with CONFIG_ROCKCHIP_FALCON_KERNEL_LOAD,
CONFIG_ROCKCHIP_FALCON_FDT_LOAD and CONFIG_ROCKCHIP_FALCON_INITRD_LOAD.
...
gzip. Note that the same setting also compresses U-Boot proper in the
regular ``u-boot.itb`` built alongside, which does not carry a kernel.
