 ...
 * Sorting full paths is a deterministic-and-cheap stand-in.
 *
 ...
 *
 * @paths:	Receives the discovered full paths, sorted ascending
 ...
 * Return: 0 on success, < 0 on error. An empty @paths is not an error;
 *	the caller turns that into -ENOENT.
 */
			     ...
			     int seq)
{
	...
	/*
	 * Record the prefix this entry was found under, so 'bootflow info' can
	 * report where it came from. Entries from different prefixes can share
	 * a partition, so this is per-entry rather than per-scan; the path was
	 * built as "<prefix>" BLS_DIR "/<name>", so the prefix is everything
	 * before BLS_DIR.
	 */
	dir = strstr(fpath, BLS_DIR);
	free(bflow->subdir);
	bflow->subdir = dir ? strndup(fpath, dir - fpath) : strdup("");
	if (!bflow->subdir)
		return log_msg_ret("dir", -ENOMEM);
